...
with tab_matriz:
    st.markdown("##### Matriz de distâncias, capacidade e decisão logística")
    ...
    for mun in MUNICIPIOS:
        ...
        decisao = "✅ Bate-volta" if bv else "🌙 Pernoite"
        if sat:
            decisao += " ⚠️"
        ...
        linhas.append({
            ...
            "Saturado?":       "⚠️ Sim" if sat else "—",
            "Decisão":         decisao,
            "_ord":            d_ida,
            "_decisao_raw":    ("✅ Bate-volta" if bv else "🌙 Pernoite"),
            "_sat":            sat,
        })
    ...
    if filtro_decisao == "✅ Bate-volta":
        linhas = [l for l in linhas if l["_decisao_raw"] == "✅ Bate-volta"]
    elif filtro_decisao == "🌙 Pernoite":
        linhas = [l for l in linhas if l["_decisao_raw"] == "🌙 Pernoite"]
    elif filtro_decisao == "⚠️ Saturado":
        linhas = [l for l in linhas if l["_sat"]]
    ...
    st.dataframe(
        ...
        use_container_width=True,
        hide_index=True,
        height=min(80 + len(linhas) * 35, 660),
    )
    ...
    st.caption(
        ...
        f"Exibindo {len(linhas)} de 21 municípios"
    )
